  ...
  const onSubmit = async (event) => {
    ...
    if (!studentId || !studentName || selectedSymptoms.length === 0) {
      toast({
        ...
        description: 'Please fill in all fields and select at least one symptom.',
        duration: 2000, 
      });
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const selectedSymptomLabels = selectedSymptoms.map(id => symptomLabels[id] || "Unknown").join(", ");

        toast({
          variant: "success",
          title: 'Data Submitted',
          description: `รหัสนักศึกษา ${studentId} ชื่อ-นามสกุล ${studentName} อาการ ${selectedSymptomLabels} ${isOtherChecked ? `อื่นๆ: ${otherSymptom}` : ''}`,
          duration: 2000, 
        });
      } else {
  ...
  return (
    <div className="mb-20">
      ...
      <div className="flex justify-center items-center min-h-screen bg-gray-100">
        <div className="bg-white shadow-md rounded-lg p-8 max-w-lg w-full mb-20">
          ...
          <form onSubmit={onSubmit}>
            ...
            <div className="flex justify-center">
              <Button className="bg-blue-700 hover:bg-blue-400" type="submit">ยืนยัน</Button>
            </div>
          </form>
        </div>
      </div>
